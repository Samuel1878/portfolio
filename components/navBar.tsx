...
export default function NavBar () {
    ...
    return(
        <div className={`fixed right-0 top-0 left-0 h-30 bg-transaprent flex items-center justify-between p-8 md:p-12 lg:p-16 z-10`}>
             <div onMouseOver={()=>setActive(true)} onMouseLeave={()=>setActive(false)} className="group cursor-pointer overflow-hidden flex flex-col items-center self-end z-10">
            ...
            <ul className="flex justify-between items-center hidden lg:flex gap-10 z-10">
                <li onMouseOver={()=>setActive(true)} onMouseLeave={()=>setActive(false)} className="text-gray-300 font-bold flex flex-col group ">
            ...
            <div className={`fixed bottom-0 flex flex-col top-0 right-0 left-0 bg-bgPrimary py-32 transition-all duration-500 ease-in-out opacity-0${open?"block opacity-100":"hidden opacity-0"} lg:hidden`}>
                <ul className={`flex flex-1 flex-col items-center justify-around h-full gap-10 pt-32 transition-all duration-500 ease-in-out  ${open?"-translate-y-10":""}`}>
                    <li className="text-l text-gray-300 hover:text-primary" onMouseOver={()=>setActive(true)} onMouseLeave={()=>setActive(false)}>
                        <Link href="/" >
                            Home
                        </Link>
                    </li>
